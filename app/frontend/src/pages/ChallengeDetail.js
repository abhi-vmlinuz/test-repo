...
import { ArrowLeft, Flag, Lightbulb, Play, CheckCircle2, Container, Sparkles, HelpCircle, Send } from 'lucide-react';
...
const ChallengeDetail = ({ user, logout }) => {
  ...
  // Calculate total points (main + questions)
      ...
      <main className="w-full px-8 lg:px-16 py-10">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className={`bg-white rounded-3xl shadow-sm overflow-hidden ${isSolved ? 'ring-2 ring-emerald-200' : ''}`}>
              ...
              <div className="p-8">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <span className="text-xl text-gray-900 font-mono font-bold">{getTotalPoints()} pts</span>
            ...
            {/* Docker instance - only show if has docker */}
            ...
            {challenge.questions && challenge.questions.length > 0 && (
              <div className="bg-white rounded-3xl shadow-sm p-8">
                <div className="flex items-center gap-3 mb-6">
                  <HelpCircle className="w-6 h-6 text-indigo-500" strokeWidth={1.5} />
                  <h2 className="text-xl font-semibold text-gray-900">Questions</h2>
                  <span className="text-gray-400 text-sm ml-auto">
                    {solvedQuestions.length} / {challenge.questions.length} completed
                  </span>
                </div>
                ...
                <div className="space-y-4">
                  {challenge.questions.map((question, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className={`border rounded-2xl p-6 transition-all ${isQuestionSolved
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-start gap-3">
                            {isQuestionSolved ? (
                              <CheckCircle2 className="w-5 h-5 text-emerald-500 mt-0.5" fill="currentColor" stroke="white" />
                            ) : (
                            ...
                            <div>
                              <p className={`font-medium ${isQuestionSolved ? 'text-emerald-700' : 'text-gray-900'}`}>
                                {question.question}
                              </p>
                              <p className={`text-sm mt-1 ${isQuestionSolved ? 'text-emerald-600' : 'text-gray-400'}`}>
            ...
            {/* Main Flag submission - only show if challenge has a main flag or no questions */}
              ...
              <div className="space-y-5">
                ...
                {challenge.questions && challenge.questions.length > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900">{challenge.questions.length}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-500">Total Points</span>
                  <span className="text-gray-900 font-mono font-semibold">{getTotalPoints()}</span>
                </div>
                <div className="flex justify-between">
