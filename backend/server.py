...
GITHUB_REDIRECT_URI = os.environ.get(
    "GITHUB_REDIRECT_URI", "https://ctf.zecurx.com/api/auth/github/callback"
)
...
GOOGLE_REDIRECT_URI = os.environ.get(
    "GOOGLE_REDIRECT_URI", "https://ctf.zecurx.com/api/auth/google/callback"
)
...
class PublicChallengeCreate(BaseModel):
    ...
    tags: List[
        str
    ] = []  # Tags for filtering/display (e.g., "web", "crypto", "forensics")
    is_published: bool = True
    # Multi-container pack support
    challenge_pack_id: Optional[str] = (
        None  # ID of the challenge pack (for docker-compose based challenges)
    )
...
CERTIFICATION_DIFFICULTY_POINTS = {"EASY": 10, "MEDIUM": 20, "HARD": 30}
...
def _sanitize_audit_value(value: Any, depth: int = 0) -> Any:
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            ...
            if key_lower in SENSITIVE_AUDIT_KEYS or any(
                s in key_lower for s in ["password", "token", "secret"]
            ):
...
async def create_session(
    conn, user_id: str, session_token: str, request: Request = None
) -> str:
    """Create a new active session for user"""
    ip_address = request.client.host if request else "unknown"
    user_agent = (
        request.headers.get("user-agent", "unknown")[:500] if request else "unknown"
    )
...
async def create_otp(
    conn, user_id: str, email: str, purpose: str = "force_logout"
) -> str:
...
async def verify_otp(
    conn, user_id: str, otp_code: str, purpose: str = "force_logout"
) -> bool:
    """Verify OTP code"""
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if (
            request.url.path.startswith(("/public", "/docs", "/openapi.json"))
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if user.get("social_links"):
                try:
                    social_links = (
                        json.loads(user["social_links"])
                        if isinstance(user["social_links"], str)
                        else (user["social_links"] or {})
                    )
                except:
                    pass
...
async def login(credentials: UserLogin, request: Request):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "DELETE FROM ctf_active_sessions WHERE user_id::text = $1", str(user["id"])
        )
        ...
        return {
            ...
            "user": {
                ...
                "role_type": user[
                    "role_type"
                ],  # Include actual role type for LMS integration
                "avatar_url": user["avatar_url"],
            },
        }
...
async def request_force_logout(data: ForceLogoutRequest):
    ...
    async with pool.acquire() as conn:
        ...
        email_sent = await send_otp_email(
            user["email"], otp_code, user["name"] or "User"
        )
...
async def verify_force_logout(data: OTPVerifyRequest, request: Request):
    ...
    async with pool.acquire() as conn:
        ...
        logger.info(
            f"Force logout completed for user {user['email']} from {request.client.host}"
        )
...
async def upload_avatar(
    ...
    async with pool.acquire() as conn:
        old_avatar = await conn.fetchval(
            "SELECT avatar_url FROM users WHERE id::text = $1", current_user["id"]
        )

        # Cleanup old local file if it exists
        if old_avatar and (
            "/uploads/avatars/" in old_avatar or "/api/uploads/avatars/" in old_avatar
        ):
            try:
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE users SET avatar_url = $1 WHERE id::text = $2",
                avatar_url,
                current_user["id"],
            )
...
async def reset_avatar(current_user: dict = Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        ...
        if (
            "/uploads/avatars/" in current_avatar
            or "/api/uploads/avatars/" in current_avatar
        ):
            try:
        ...
        new_avatar = (
            user.get("google_avatar")
            or user.get("github_avatar")
            or user.get("avatar")  # LMS avatar field
            or None
        )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if token and username:
                return {"connected": True, "username": username["value"]}
...
async def github_oauth_disconnect(admin: dict = Depends(require_admin)):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "DELETE FROM admin_settings WHERE key = 'github_oauth_token'"
            )
            await conn.execute(
                "DELETE FROM admin_settings WHERE key = 'github_oauth_username'"
            )
...
async def _handle_github_login_callback(code: str, state: Optional[str] = None):
    ...
    if not GITHUB_CLIENT_ID or not GITHUB_CLIENT_SECRET:
        return HTMLResponse(
            content=_github_error_html("GitHub OAuth not configured"), status_code=500
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if token_response.status_code != 200:
                logger.error(f"GitHub token exchange failed: {token_response.text}")
                return HTMLResponse(
                    content=_github_error_html("Failed to authenticate with GitHub"),
                    status_code=400,
                )
            ...
            if not access_token:
                return HTMLResponse(
                    content=_github_error_html("No access token received"),
                    status_code=400,
                )
            ...
            if user_response.status_code != 200:
                return HTMLResponse(
                    content=_github_error_html("Failed to fetch GitHub user info"),
                    status_code=400,
                )
            ...
            if not github_email:
                ...
                if emails_response.status_code == 200:
                    ...
                    github_email = primary_email or (
                        emails[0]["email"] if emails else None
                    )

            if not github_email:
                return HTMLResponse(
                    content=_github_error_html(
                        "Could not retrieve email from GitHub. Please make sure your email is public or use password login."
                    ),
                    status_code=400,
                )
            ...
            async with pool.acquire() as conn:
                ...
                if existing_by_github:
                    ...
                    if (
                        existing_by_github["github_avatar"] != github_avatar
                        or existing_by_github["avatar_url"] is None
                    ):
                        await conn.execute(
                    ...
                    if existing_by_email:
                        ...
                        logger.info(
                            f"Linked GitHub {github_login} to existing user {github_email}"
                        )
                    else:
                        # 3. Create new user
                        # Get CTF_USER role
                        role = await conn.fetchrow(
                            "SELECT id FROM \"Role\" WHERE type = 'CTF_USER'"
                        )
                        if not role:
                            role_id = generate_uuid()
                            await conn.execute(
                        ...
                        logger.info(
                            f"Created new user from GitHub: {github_login} ({github_email})"
                        )
...
async def google_oauth_callback(code: str, state: Optional[str] = None):
    ...
    if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET:
        return HTMLResponse(
            content=_google_error_html("Google OAuth not configured"), status_code=500
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if token_response.status_code != 200:
                ...
                return HTMLResponse(
                    content=_google_error_html("Failed to authenticate with Google"),
                    status_code=400,
                )
            ...
            if not access_token:
                return HTMLResponse(
                    content=_google_error_html("No access token received"),
                    status_code=400,
                )
            ...
            if user_response.status_code != 200:
                return HTMLResponse(
                    content=_google_error_html("Failed to fetch Google user info"),
                    status_code=400,
                )
            ...
            if not google_email:
                return HTMLResponse(
                    content=_google_error_html("Could not retrieve email from Google"),
                    status_code=400,
                )
            ...
            async with pool.acquire() as conn:
                ...
                if existing_by_google:
                    ...
                    # Only update if something changed to avoid redundant writes
                    if (
                        existing_by_google["google_avatar"] != google_avatar
                        or existing_by_google["avatar_url"] is None
                    ):
                        await conn.execute(
...
async def get_challenges(
    ...
    async with pool.acquire() as conn:
        ...
        for ch in challenges:
            ...
            if ch.get("questions"):
                q_data = (
                    json.loads(ch["questions"])
                    if isinstance(ch["questions"], str)
                    else ch["questions"]
                )
                questions = [
                    {"question": q.get("question"), "points": q.get("points", 25)}
                    for q in q_data
                ]
            ...
            result.append(
                {
                    ...
                    "difficulty": ch["difficulty"].lower()
                    if ch["difficulty"]
                    else "medium",
...
async def get_challenge(
    ...
    async with pool.acquire() as conn:
        ...
        hints_data = (
            json.loads(challenge["hints"])
            if isinstance(challenge["hints"], str)
            else (challenge["hints"] or [])
        )
        hints_used = (
            list(progress["hintsUsed"]) if progress else []
        )  # Keep for user_progress
        hints = []
        ...
        # Parse questions
        questions_data = (
            json.loads(challenge["questions"])
            if isinstance(challenge["questions"], str)
            else (challenge["questions"] or [])
        )
        ...
        if challenge.get("tags"):
            tags = (
                json.loads(challenge["tags"])
                if isinstance(challenge["tags"], str)
                else challenge["tags"]
            )
...
async def submit_flag(
    submission: FlagSubmit, current_user: dict = Depends(get_current_user)
):
...
async def submit_question(
    submission: QuestionSubmit, current_user: dict = Depends(get_current_user)
):
...
async def _auto_destroy_nexus_session(user_id: str, challenge_id: str) -> None:
    ...
    if user_id in nexus_sessions and challenge_id in nexus_sessions[user_id]:
        ...
        logger.info(
            f"[AUTO-DESTROY] Found session {session_id} in cache for user {user_id}"
        )
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                if row:
                    ...
                    logger.info(
                        f"[AUTO-DESTROY] Found session {session_id} in DB for user {user_id}"
                    )
        except Exception as e:
    ...
    if not session_id:
        logger.info(
            f"[AUTO-DESTROY] No active session found for user {user_id}, challenge {challenge_id} - skipping"
        )
        return
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code in [200, 404]:
                logger.info(
                    f"[AUTO-DESTROY] Session {session_id} terminated (status={resp.status_code})"
                )
            else:
                logger.warning(
                    f"[AUTO-DESTROY] Unexpected status {resp.status_code} for session {session_id}: {resp.text}"
                )
    except Exception as e:
        logger.error(
            f"[AUTO-DESTROY] Failed to call Nexus for session {session_id}: {e}"
        )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            logger.info(
                f"[AUTO-DESTROY] nexus_usage marked 'completed' for session {session_id}"
            )
    except Exception as e:
        logger.warning(
            f"[AUTO-DESTROY] Failed to update nexus_usage for session {session_id}: {e}"
        )
    ...
    if user_id in nexus_sessions and challenge_id in nexus_sessions.get(user_id, {}):
        ...
        logger.info(
            f"[AUTO-DESTROY] Removed session {session_id} from nexus_sessions cache"
        )
...
async def unlock_hint(
    hint_request: HintRequest, current_user: dict = Depends(get_current_user)
):
...
async def get_public_profile(user_id: str):
    ...
    async with pool.acquire() as conn:
        ...
        score = user["score"] or 0
        completion = (
            (challenges_solved / total_challenges * 100) if total_challenges > 0 else 0
        )
        ...
        if completion >= 50:
            achievements.append(
                {
                    "name": "Halfway There",
                    "description": "Completed 50% of challenges",
                    "earned": True,
                }
            )
        else:
...
async def admin_get_categories(admin: dict = Depends(require_admin)):
    ...
    async with pool.acquire() as conn:
        categories = await conn.fetch(
            'SELECT * FROM ctf_categories ORDER BY "orderIndex"'
        )
        return [dict(c) for c in categories]
...
async def admin_create_category(
    data: CategoryCreate, admin: dict = Depends(require_admin)
):
...
async def admin_delete_category(category_id: str, admin: dict = Depends(require_admin)):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM ctf_categories WHERE id::text = $1", category_id
        )
        return {"success": True}
...
async def admin_get_public_challenges(
    category_id: Optional[str] = None, admin: dict = Depends(require_admin)
):
...
async def admin_create_public_challenge(
    data: PublicChallengeCreate, admin: dict = Depends(require_admin)
):
    ...
    async with pool.acquire() as conn:
        ...
        if existing:
            raise HTTPException(
                status_code=409,
                detail=f"A challenge with the title '{data.title}' already exists",
            )
        ...
        questions = [
            {"question": q.question, "flag": q.flag, "points": q.points}
        ...
        """,
            ...
            data.flag,
            data.docker_image,
            None,  # dockerCommand is deprecated
            json.dumps(hints),
...
async def admin_delete_public_challenge(
    challenge_id: str, admin: dict = Depends(require_admin)
):
    """Delete a public challenge"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM ctf_public_challenges WHERE id::text = $1", challenge_id
        )
        return {"success": True}
...
async def admin_link_lms_course(
    lms_course_id: str, color: str = "gray", admin: dict = Depends(require_admin)
):
    ...
    async with pool.acquire() as conn:
        ...
        existing = await conn.fetchrow(
            'SELECT id FROM ctf_courses WHERE "lmsCourseId" = $1', lms_course_id
        )
        if existing:
...
async def validate_certification_pool(challenge_ids: List[str], conn) -> tuple:
    ...
    for c in challenges:
        ...
        if difficulty not in CERTIFICATION_DIFFICULTY_POINTS:
            return (
                False,
                f"Challenge '{c['title']}' has invalid difficulty '{difficulty}' (only EASY, MEDIUM, HARD allowed)",
                0,
                [],
            )
    ...
    if total_points != 120:
        return (
            False,
            f"Pool must total 120 points (has {total_points})",
            total_points,
            details,
        )
...
async def admin_get_available_certification_challenges(
    ...
    async with pool.acquire() as conn:
        ...
        print(
            f"[DEBUG] admin_get_available_certification_challenges: Total rows from DB: {len(challenges)}"
        )
        ...
        for c in challenges:
            ...
            print(
                f"[DEBUG] Challenge: id={challenge_id}, title={c['title']}, difficulty={difficulty}, is_published={c['is_published']}"
            )

            result[difficulty.lower()].append(challenge_data)

        print(
            f"[DEBUG] Result counts: easy={len(result['easy'])}, medium={len(result['medium'])}, hard={len(result['hard'])}"
        )
            ...
            "final_exams": [
                {
                    ...
                    "certification_config_id": str(e["certification_config_id"])
                    if e["certification_config_id"]
                    else None,
...
async def admin_create_certification_exam(
    data: CertificationExamConfigCreate, admin: dict = Depends(require_admin)
):
    """
    ...
    async with pool.acquire() as conn:
        ...
        if existing:
            raise HTTPException(
                status_code=409,
                detail="This LMS Final Exam already has a certification exam configuration",
            )

        # Validate Pool A
        valid_a, error_a, points_a, details_a = await validate_certification_pool(
            data.pool_a_challenge_ids, conn
        )
        if not valid_a:
            raise HTTPException(
                status_code=400, detail=f"Pool A validation failed: {error_a}"
            )

        # Validate Pool B
        valid_b, error_b, points_b, details_b = await validate_certification_pool(
            data.pool_b_challenge_ids, conn
        )
        if not valid_b:
            raise HTTPException(
                status_code=400, detail=f"Pool B validation failed: {error_b}"
            )

        # Validate Pool C
        valid_c, error_c, points_c, details_c = await validate_certification_pool(
            data.pool_c_challenge_ids, conn
        )
        if not valid_c:
            raise HTTPException(
                status_code=400, detail=f"Pool C validation failed: {error_c}"
            )
        ...
        """,
            ...
            data.name,
            data.lms_final_exam_id,
            data.pool_a_challenge_ids,
            data.pool_b_challenge_ids,
            data.pool_c_challenge_ids,
            data.global_duration_hours,
            data.ctf_duration_hours,
            data.report_duration_hours,
            admin["id"],
        )
...
async def admin_get_certification_exam(
    config_id: str, admin: dict = Depends(require_admin)
):
    ...
    async with pool.acquire() as conn:
        ...
        async def get_pool_details(challenge_ids):
            ...
            for cid in challenge_ids:
                ...
                if c:
                    difficulty = (
                        c["difficulty"].upper() if c["difficulty"] else "MEDIUM"
                    )
                    result.append(
                        {
                            "id": str(c["id"]),
                            ...
                            "difficulty": difficulty,
                            "category": c["category"] or "Uncategorized",
                            "points": CERTIFICATION_DIFFICULTY_POINTS.get(
                                difficulty, 20
                            ),
                        }
                    )
        ...
        return {
            ...
            "created_at": config["createdAt"].isoformat()
            if config["createdAt"]
            else None,
...
async def admin_update_certification_exam(
    config_id: str,
    data: CertificationExamConfigUpdate,
    admin: dict = Depends(require_admin),
):
    ...
    async with pool.acquire() as conn:
        ...
        config = await conn.fetchrow(
            "SELECT * FROM certification_exam_configs WHERE id::text = $1", config_id
        )
        if not config:
            raise HTTPException(status_code=404, detail="Certification exam not found")
        ...
        if data.pool_a_challenge_ids is not None:
            valid, error, _, _ = await validate_certification_pool(
                data.pool_a_challenge_ids, conn
            )
            if not valid:
                raise HTTPException(
                    status_code=400, detail=f"Pool A validation failed: {error}"
                )
        ...
        if data.pool_b_challenge_ids is not None:
            ...
            if not valid:
                raise HTTPException(
                    status_code=400, detail=f"Pool B validation failed: {error}"
                )
        ...
        if data.pool_c_challenge_ids is not None:
            ...
            if not valid:
                raise HTTPException(
                    status_code=400, detail=f"Pool C validation failed: {error}"
                )
...
async def admin_delete_certification_exam(
    config_id: str, admin: dict = Depends(require_admin)
):
    ...
    async with pool.acquire() as conn:
        # Check if exam exists
        config = await conn.fetchrow(
            "SELECT id FROM certification_exam_configs WHERE id::text = $1", config_id
        )
        if not config:
            raise HTTPException(status_code=404, detail="Certification exam not found")
        ...
        if attempt_count > 0:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot delete exam with {attempt_count} existing attempts",
            )

        await conn.execute(
            "DELETE FROM certification_exam_configs WHERE id::text = $1", config_id
        )
        return {"message": "Certification exam deleted successfully"}


@api_router.put("/admin/certification-exams/{config_id}/publish")
async def admin_publish_certification_exam(
    config_id: str, admin: dict = Depends(require_admin)
):
    ...
    async with pool.acquire() as conn:
        config = await conn.fetchrow(
            'SELECT id, "isPublished" FROM certification_exam_configs WHERE id::text = $1',
            config_id,
        )
        if not config:
            raise HTTPException(status_code=404, detail="Certification exam not found")
        ...
        return [
            {
                ...
                "report_score": float(a["reportTotalScore"])
                if a["reportTotalScore"]
                else None,
                ...
                "global_expires_at": a["globalExpiresAt"].isoformat()
                if a["globalExpiresAt"]
                else None,
                "lab_started_at": a["labStartedAt"].isoformat()
                if a["labStartedAt"]
                else None,
                "lab_expires_at": a["labExpiresAt"].isoformat()
                if a["labExpiresAt"]
                else None,
                "lab_completed_at": a["labCompletedAt"].isoformat()
                if a["labCompletedAt"]
                else None,
                "report_unlocked_at": a["reportUnlockedAt"].isoformat()
                if a["reportUnlockedAt"]
                else None,
                "report_uploaded_at": a["reportUploadedAt"].isoformat()
                if a["reportUploadedAt"]
                else None,
                "report_graded_at": a["reportGradedAt"].isoformat()
                if a["reportGradedAt"]
                else None,
            }
            for a in attempts
...
async def admin_create_new_category(
    data: CategoryCreate, admin: dict = Depends(require_admin)
):
    """Create a new category"""
    pool = await Database.get_pool()
    ...
    async with pool.acquire() as conn:
        ...
        if data.role is not None:
            ...
            if admin.get("role") != "superadmin":
                raise HTTPException(
                    status_code=403, detail="Only superadmin can change roles"
                )
            ...
            role_row = await conn.fetchrow(
                'SELECT id FROM "Role" WHERE type = $1', role_type
            )
            if role_row:
                await conn.execute(
                    """
...
async def admin_reset_user_progress(user_id: str, admin: dict = Depends(require_admin)):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            'DELETE FROM ctf_public_progress WHERE "userId"::text = $1', user_id
        )
...
async def admin_delete_user(user_id: str, admin: dict = Depends(require_admin)):
    ...
    async with pool.acquire() as conn:
        ...
        if target_role == "SUPERADMIN":
            raise HTTPException(
                status_code=403, detail="Cannot delete a Superadmin account"
            )

        if target_role in ["ADMIN", "INSTRUCTOR"] and admin_role != "superadmin":
            raise HTTPException(
                status_code=403, detail="Only Superadmin can delete other Admins"
            )
...
async def admin_get_submissions(
    limit: int = 200, solved_only: bool = False, admin: dict = Depends(require_admin)
):
...
async def admin_create_challenge(
    ...
    async with pool.acquire() as conn:
        ...
        """,
            ...
            data.title,
            data.description,
            data.difficulty.upper(),
            data.points,
            data.flag,
            data.author,
            data.docker_image,
            None,  # dockerCommand deprecated
            json.dumps(hints),
            json.dumps(questions),
            json.dumps(tags),
            json.dumps(ports),
...
async def admin_update_challenge(
    challenge_id: str, data: PublicChallengeCreate, admin: dict = Depends(require_admin)
):
    """Update a challenge"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
                    ...
                    if ":" in port_str:
                        ...
                        try:
                            ...
                            if "-" in internal:
                                ...
                                for p in range(
                                    start_port, min(end_port + 1, start_port + 10)
                                ):  # Limit range
...
def detect_all_ports_from_zip(zip_file_contents: dict) -> tuple[List[int], dict]:
    ...
    """
    all_ports = []
    detection_info = {"sources": [], "details": {}}

    # 1. Parse main Dockerfile
    if zip_file_contents.get("dockerfile_content"):
        dockerfile_ports = parse_expose_from_dockerfile(
            zip_file_contents["dockerfile_content"]
        )
        if dockerfile_ports:
    ...
    if zip_file_contents.get("docker_compose_content"):
        compose_ports = parse_ports_from_docker_compose(
            zip_file_contents["docker_compose_content"]
        )
        if compose_ports:
...
async def analyze_dockerfile_ports(
    ...
    if not ports and image_url:
        ...
        if "nginx" in image_lower or "apache" in image_lower or "httpd" in image_lower:
            ...
            "python" in image_lower or "flask" in image_lower or "django" in image_lower
        ):
            ports = [8000]
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "r") as zf:
            ...
            for info in zf.infolist():
                ...
                if basename == "dockerfile" and normalized_name.lower() not in [
                    "dockerfile",
                    "dockerfile/",
                ]:
                    try:
                ...
                if (
                    ...
                    or normalized_name.lower().endswith("/docker-compose.yml")
                    or normalized_name.lower().endswith("/docker-compose.yaml")
                ):
                    try:
            ...
            detected_ports, port_detection_info = detect_all_ports_from_zip(
                zip_contents
            )
            ...
            return {
                ...
                "dockerfile_content": dockerfile_content,
                "docker_compose_content": docker_compose_content,
                "additional_dockerfiles": list(
                    additional_dockerfiles.keys()
                ),  # Show paths where extra Dockerfiles found
                "detected_ports": detected_ports,
                "port_detection_info": port_detection_info,  # Details on where ports came from
                "has_wrapper_folder": bool(wrapper_prefix),
                "wrapper_folder": wrapper_prefix.rstrip("/")
                if wrapper_prefix
                else None,
            }
...
async def upload_challenge_artifact(
    ...
    try:
        ...
        if file_size > MAX_SIZE:
            raise HTTPException(
                status_code=413, detail="File too large. Maximum size is 300MB."
            )
        ...
        return {"id": str(artifact_id), "filename": clean_filename, "size": file_size}
    except Exception as e:
...
async def delete_challenge_artifact(
    artifact_id: str, admin: dict = Depends(require_admin)
):
    """Delete an artifact"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT file_path FROM ctf_challenge_artifacts WHERE id::text = $1",
        ...
        await conn.execute(
            "DELETE FROM ctf_challenge_artifacts WHERE id::text = $1",
            uuid.UUID(artifact_id),
        )
        return {"success": True}
...
async def download_artifact(artifact_id: str):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT file_path, filename, mime_type FROM ctf_challenge_artifacts WHERE id::text = $1",
            uuid.UUID(artifact_id),
        )
        if not row:
            raise HTTPException(status_code=404, detail="Artifact not found")
...
async def import_artifact_from_github(
    ...
    if not github_token:
        raise HTTPException(
            status_code=400,
            detail="GitHub token not configured. Connect GitHub in Image Registry first.",
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 404:
                raise HTTPException(
                    status_code=404, detail=f"File not found: {path} in {repo}/{branch}"
                )
            elif resp.status_code != 200:
                raise HTTPException(
                    status_code=resp.status_code,
                    detail=f"GitHub error: {resp.status_code}",
                )
            ...
            logger.info(
                f"Imported artifact from GitHub: {repo}/{path} -> {clean_filename}"
            )
...
async def get_zip_info(
    ...
    if not file.filename.endswith(".zip"):
        raise HTTPException(
            status_code=400, detail="Only ZIP files are supported for preview"
        )
    ...
    try:
        ...
        with zipfile.ZipFile(temp_path, "r") as zf:
            ...
            for name in all_files[:100]:  # Limit to first 100 entries
                ...
                files.append({"name": name, "size": info.file_size, "is_dir": is_dir})
            ...
            for name in all_files:
                ...
                if basename == "dockerfile":
                    if not has_dockerfile:
                        has_dockerfile = True
                        try:
                            dockerfile_content = zf.read(name).decode(
                                "utf-8", errors="ignore"
                            )[:5000]
                        except:
                            pass
                    ...
                    if not has_docker_compose:
                        ...
                        try:
                            docker_compose_content = zf.read(name).decode(
                                "utf-8", errors="ignore"
                            )[:5000]
                        except:
                            pass
            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    for pkg in packages:
                        ...
                        image_url = (
                            f"ghcr.io/{owner.lower()}/{pkg['name'].lower()}:latest"
                        )
                ...
                packs.append(
                    {
                        ...
                        else json.loads(row["images"]),
                        "combined_ports": row["combined_ports"]
                        if isinstance(row["combined_ports"], list)
                        else json.loads(row["combined_ports"]),
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            username = (
                username_row["value"]
                if username_row
                else os.environ.get("GHCR_USERNAME", "")
            )
...
async def test_ghcr_connection(
    config: GHCRConfig, admin: dict = Depends(require_admin)
):
...
async def admin_github_repos(admin: dict = Depends(require_admin)):
    ...
    if not ghcr_token:
        raise HTTPException(
            status_code=400, detail="GitHub not connected. Configure GHCR first."
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code != 200:
                raise HTTPException(
                    status_code=resp.status_code, detail="Failed to fetch repos"
                )
...
async def admin_github_repo_contents(
    repo: str, path: str = "", admin: dict = Depends(require_admin)
):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 404:
                raise HTTPException(
                    status_code=404, detail="Repository or path not found"
                )
            if resp.status_code != 200:
                raise HTTPException(
                    status_code=resp.status_code, detail="Failed to fetch contents"
                )
...
async def admin_github_preview_folder(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code != 200:
                raise HTTPException(
                    status_code=resp.status_code, detail="Failed to fetch folder"
                )
...
async def build_image_from_github(data: dict, admin: dict = Depends(require_admin)):
    ...
    if not repo or not image_name:
        raise HTTPException(
            status_code=400, detail="Repository and image name required"
        )
    ...
    try:
        ...
        stdout, stderr = await asyncio.wait_for(
            clone_process.communicate(), timeout=120
        )
        ...
        has_docker_compose = (
            (context_dir / "docker-compose.yml").exists()
            or (context_dir / "docker-compose.yaml").exists()
            or (context_dir / "compose.yml").exists()
            or (context_dir / "compose.yaml").exists()
        )
        ...
        if not has_dockerfile and not has_docker_compose:
            raise HTTPException(
                status_code=400,
                detail="No Dockerfile or docker-compose.yml found in folder",
            )
        ...
        if has_docker_compose:
            ...
            for service_name, service_config in services.items():
                ...
                if service_build:
                    ...
                    stdout, stderr = await asyncio.wait_for(
                        build_process.communicate(), timeout=600
                    )

                    if build_process.returncode != 0:
                        logger.error(
                            f"Build failed for {service_name}: {stderr.decode()}"
                        )
                        continue
            ...
            if not built_images:
                raise HTTPException(
                    status_code=500, detail="No services could be built"
                )
            ...
            try:
                ...
                async with pool.acquire() as conn:
                    ...
                    """,
                        ...
                        clean_name,
                        image_name,
                        json.dumps(built_images),
                        json.dumps(list(set(all_ports))),
                        compose_content,
                    )
            ...
            if build_process.returncode != 0:
                raise HTTPException(
                    status_code=500, detail=f"Build failed: {stderr.decode()[:500]}"
                )
...
async def build_docker_image(
    ...
    try:
        ...
        if (
            len(subdirs) == 1
            and not (extracted_dir / "Dockerfile").exists()
            and not (extracted_dir / "docker-compose.yml").exists()
        ):
        ...
        if compose_path:
            ...
            if not services:
                raise HTTPException(
                    status_code=400, detail="No services found in docker-compose.yml"
                )
            ...
            for service_name, service_config in services.items():
                ...
                if build_config:
                    ...
                    if not dockerfile_path.exists():
                        logger.warning(
                            f"Dockerfile not found for service {service_name}: {dockerfile_path}"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.info(
                            f"Built and pushed {service_name} with ports {service_ports}"
                        )

                    except Exception as build_err:
                        logger.error(
                            f"Failed to build service {service_name}: {build_err}"
                        )
            ...
            if not built_images:
                raise HTTPException(
                    status_code=400,
                    detail="No services could be built from docker-compose.yml",
                )
            ...
            try:
                ...
                async with pool.acquire() as conn:
                    ...
                    """,
                        ...
                        image_name,
                        json.dumps(built_images),
                        json.dumps(all_ports),
                        compose_content,
                    )

                    logger.info(
                        f"Stored challenge pack '{clean_name}' with {len(built_images)} images"
                    )
        ...
        # =====================================
            ...
            if not dockerfile_path:
                raise HTTPException(
                    status_code=400,
                    detail="No Dockerfile or docker-compose.yml found in ZIP",
                )
            ...
            try:
                image, build_logs = docker_client.images.build(
                    path=str(extracted_dir), tag=full_image_name, rm=True
                )
                ...
                shutil.rmtree(build_dir, ignore_errors=True)
                raise HTTPException(
                    status_code=500, detail=f"Build failed: {str(build_error)}"
                )
...
async def cleanup_orphaned_images(admin: dict = Depends(require_admin)):
    ...
    if not ghcr_token or not ghcr_username:
        return {
            "success": False,
            "message": "GHCR not configured. Cannot determine orphaned images.",
        }
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                return {
                    "success": False,
                    "message": f"GHCR API error: {resp.status_code}",
                }
    except Exception as e:
...
async def delete_challenge_pack(pack_id: str, admin: dict = Depends(require_admin)):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            # Delete the pack
            await conn.execute(
                "DELETE FROM challenge_packs WHERE id::text = $1", pack_id
            )

            logger.info(
                f"Challenge pack '{pack['display_name']}' ({pack_id}) deleted by admin {admin.get('email')}"
            )
...
async def get_image_metadata(
    image_url: str = Form(...), admin: dict = Depends(require_admin)
):
    ...
    try:
        ...
        return {"found": False, "image_url": image_url, "ports": [], "message": str(e)}
...
async def admin_create_challenge_with_docker(
    ...
    try:
        ...
        if not dockerfile_path.exists():
            ...
            raise HTTPException(
                status_code=400, detail="Dockerfile not found at root of ZIP"
            )
        ...
        async with pool.acquire() as conn:
            ...
            """,
                ...
                data.get("difficulty", "medium").upper(),
                data.get("points", 100),
                data.get("flag", ""),
                docker_image,
                None,  # dockerCommand deprecated
        ...
        return {
            ...
            if docker_image and not docker_image.startswith("pending")
            else "pending",
        }
...
async def get_student_enrollments(current_user: dict = Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        ...
        for lms in lms_enrollments:
            ...
            logger.info(
                f"Auto-enrolled user {user_id} in CTF course {lms['course_name']} (synced from LMS)"
            )
            ...
            result.append(
                {
                    ...
                    "enrolled_at": e["enrolledAt"].isoformat()
                    if e["enrolledAt"]
                    else None,
...
async def join_course(
    data: JoinCourseRequest, current_user: dict = Depends(get_current_user)
):
...
async def get_student_course(
    ...
    async with pool.acquire() as conn:
        ...
        return {**dict(course), "modules": [dict(m) for m in modules]}
        ...
        for ch in challenges:
            ...
            if hints:
                if isinstance(hints, str):
                    hints = json.loads(hints)
                ch_dict["hints"] = [
                    {"text": h.get("text", ""), "cost": 0} for h in hints
                ]
            result_challenges.append(ch_dict)

        return {**dict(module), "challenges": result_challenges}


@api_router.get("/student/challenges/{challenge_id}")
async def get_student_challenge(
    challenge_id: str, current_user: dict = Depends(get_current_user)
):
...
async def submit_student_flag(
    submission: StudentFlagSubmit, current_user: dict = Depends(get_current_user)
):
...
async def unlock_student_hint(
    challenge_id: str, hint_index: int, current_user: dict = Depends(get_current_user)
):
...
async def admin_dashboard(admin: dict = Depends(require_admin)):
    ...
    async with pool.acquire() as conn:
        ...
        stats["total_submissions"] = (
            await conn.fetchval("SELECT COUNT(*) FROM ctf_public_progress") or 0
        )
    ...
    async with pool.acquire() as conn:
        ...
        if data.lms_course_id:
            ...
            if existing:
                raise HTTPException(
                    status_code=400,
                    detail="This LMS course already has a linked CTF course",
                )
...
async def admin_update_course(
    course_id: str, data: CourseUpdate, admin: dict = Depends(require_admin)
):
    """Update a CTF course"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
...
async def admin_delete_course(course_id: str, admin: dict = Depends(require_admin)):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute(
            "DELETE FROM courses WHERE id::text = $1", ctf_course["lmsCourseId"]
        )
...
async def admin_get_challenges(
    module_id: Optional[str] = None, admin: dict = Depends(require_admin)
):
...
@api_router.post("/admin/challenges")
async def admin_create_challenge(
    data: StudentChallengeCreate, admin: dict = Depends(require_admin)
):
    """Create a new challenge"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        ...
        flags = [
            {"flag": f.flag, "points": f.points, "description": f.description}
            for f in data.flags
        ]
...
@api_router.put("/admin/challenges/{challenge_id}")
async def admin_update_challenge(
    challenge_id: str, data: dict, admin: dict = Depends(require_admin)
):
    """Update a challenge"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
...
async def admin_create_or_update_quiz(
    module_id: str, data: dict, admin: dict = Depends(require_admin)
):
    """Create or update a module quiz"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        existing = await conn.fetchrow(
            "SELECT id FROM module_quizzes WHERE module_id = $1 AND is_final_quiz = FALSE",
            module_id,
        )
        if existing:
            await conn.execute(
                ...
                WHERE id::text = $6
            """,
                data.get("title", "Module Quiz"),
                data.get("description", ""),
                data.get("time_limit", 3600),
                data.get("passing_percentage", 80),
            ...
            module = await conn.fetchrow(
                'SELECT "courseId" FROM modules WHERE id::text = $1', module_id
            )
...
async def admin_get_module_quiz(module_id: str, admin: dict = Depends(require_admin)):
    ...
    async with pool.acquire() as conn:
        quiz = await conn.fetchrow(
            "SELECT * FROM module_quizzes WHERE module_id = $1 AND is_final_quiz = FALSE",
            module_id,
        )
        if not quiz:
        ...
        questions = await conn.fetch(
            "SELECT * FROM module_quiz_questions WHERE quiz_id = $1 ORDER BY order_index",
            quiz["id"],
        )
        return {
...
async def admin_add_question(
    quiz_id: str, data: dict, admin: dict = Depends(require_admin)
):
    """Add a question to a quiz"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        max_order = await conn.fetchval(
            "SELECT COALESCE(MAX(order_index), 0) FROM module_quiz_questions WHERE quiz_id = $1",
            uuid.UUID(quiz_id),
        )
...
async def admin_update_question(
    quiz_id: str, question_id: str, data: dict, admin: dict = Depends(require_admin)
):
    ...
    async with pool.acquire() as conn:
        ...
        if updates:
            ...
            await conn.execute(
                f"UPDATE module_quiz_questions SET {', '.join(updates)} WHERE id = ${idx}",
                *values,
            )
        return {"success": True}
...
async def admin_delete_question(
    quiz_id: str, question_id: str, admin: dict = Depends(require_admin)
):
    """Delete a quiz question"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        await conn.execute(
            "DELETE FROM module_quiz_questions WHERE id::text = $1",
...
async def student_get_module_quiz(
    module_id: str, current_user: dict = Depends(get_current_user)
):
    ...
    async with pool.acquire() as conn:
        ...
        points_per_question = (
            round(100 / total_questions, 2) if total_questions > 0 else 0
        )
...
async def student_start_quiz(
    quiz_id: str, current_user: dict = Depends(get_current_user)
):
    """Start a quiz attempt"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        quiz = await conn.fetchrow(
            "SELECT * FROM module_quizzes WHERE id::text = $1", uuid.UUID(quiz_id)
        )
        if not quiz:
        ...
        if quiz["is_final_quiz"]:
            ...
            if last_attempt and last_attempt["attempt_number"] >= 3:
                ...
                if (
                    ...
                    and datetime.now() >= last_attempt["cooldown_until"]
                ):
                    ...
                    raise HTTPException(
                        status_code=429,
                        detail="Maximum retries reached. Wait for cooldown.",
                    )
        ...
        total_questions = await conn.fetchval(
            "SELECT COUNT(*) FROM module_quiz_questions WHERE quiz_id = $1",
            uuid.UUID(quiz_id),
        )
...
async def student_submit_quiz(
    quiz_id: str, data: dict, current_user: dict = Depends(get_current_user)
):
    ...
    async with pool.acquire() as conn:
        ...
        questions = await conn.fetch(
            "SELECT id, correct_answer FROM module_quiz_questions WHERE quiz_id = $1",
            uuid.UUID(quiz_id),
        )
...
async def student_get_quiz_results(
    quiz_id: str, current_user: dict = Depends(get_current_user)
):
...
async def student_get_final_quiz(
    course_id: str, current_user: dict = Depends(get_current_user)
):
    ...
    async with pool.acquire() as conn:
        ...
        final_quiz = await conn.fetchrow(
            "SELECT * FROM module_quizzes WHERE course_id = $1 AND is_final_quiz = TRUE",
            course_id,
        )
        ...
        if not final_quiz:
            ...
            if len(all_questions) < 5:
                return {
                    "quiz": None,
                    "message": "Not enough questions across modules for a final quiz",
                }
        ...
        total_questions = len(questions)
        points_per_question = (
            round(30 / total_questions, 2) if total_questions > 0 else 0
        )
...
async def student_get_certification_exams(
    ...
    async with pool.acquire() as conn:
        ...
        attempts = await conn.fetch(
            ...
            JOIN certification_exam_configs cec ON cea."examConfigId" = cec.id
            WHERE cea."userId"::text = $1
            ORDER BY cea."examConfigId", cea."redeemedAt" DESC
        """,
            current_user["id"],
        )
        ...
        for a in attempts:
            ...
            lab_remaining = (
                calculate_time_remaining(a["labExpiresAt"])
                if a["labExpiresAt"]
                else None
            )
            ...
            result.append(
                {
                    ...
                    "components": {
                        ...
                        "report": {
                            ...
                            "score": float(a["reportTotalScore"])
                            if a["reportTotalScore"]
                            else None,
                        },
                    },
                    ...
                    "passed": a["passed"],
                    "certification_level": a["certificationLevel"],
                    "redeemed_at": a["redeemedAt"].isoformat()
                    if a["redeemedAt"]
                    else None,
                }
            )
...
async def student_start_certification_lab(
    ...
    async with pool.acquire() as conn:
        ...
        if not attempt:
            raise HTTPException(
                status_code=404, detail="Certification exam attempt not found"
            )

        if not attempt["isPublished"]:
            raise HTTPException(
                status_code=400, detail="This certification exam is not yet published"
            )

        # Check if MCQ is completed
        if attempt["status"] == "MCQ_PENDING":
            raise HTTPException(
                status_code=400, detail="You must complete the MCQ component first"
            )
        ...
        if now >= global_expires:
            ...
            raise HTTPException(
                status_code=400, detail="Your 48-hour window has expired"
            )
...
async def student_get_lab_by_config(
    ...
    async with pool.acquire() as conn:
        ...
        randomized_order = attempt["labChallengeOrder"] or list(
            range(len(challenge_ids))
        )
        ordered_challenges = []
        total_points = 0
        earned_points = 0
        ...
        for idx in randomized_order:
            ...
            tasks_raw = (
                json.loads(c["questions"])
                if isinstance(c["questions"], str)
                else (c["questions"] or [])
            )
                ...
                tasks = [
                    {
                        ...
                        "points": pts_per_task + (1 if i < remainder else 0),
                    }
            ...
            is_solved = (
                solved_entry is not None and solved_entry.get("solved_at") is not None
            )
            solved_at = solved_entry.get("solved_at") if solved_entry else None
            questions_solved = (
                solved_entry.get("questions_solved", []) if solved_entry else []
            )
            # Backwards compatibility: also check old field name
            if not questions_solved and solved_entry:
                questions_solved = solved_entry.get("tasks_solved", [])

            # Total challenge points = cert_points (difficulty only)
            challenge_total_pts = cert_points
            total_points += challenge_total_pts
            if is_solved:
                # Points only awarded when ALL questions are solved
            ...
            ordered_challenges.append(
                {
                    ...
                    "category": c["category"] or "Uncategorized",
                    "has_docker": bool(
                        c["hasDocker"] or c["dockerImage"] or c["isMultiContainer"]
                    ),
                    "docker_image": c["dockerImage"],
        ...
        can_upload_report = (
            attempt["reportUnlockedAt"] is not None or lab_score >= threshold
        )
...
async def student_get_certification_challenges(
    ...
    async with pool.acquire() as conn:
        ...
        if isinstance(solved_challenges, str):
            solved_challenges = json.loads(solved_challenges)
        solved_ids = {
            c["challenge_id"] for c in solved_challenges if c.get("solved_at")
        }
        ...
        report_unlocked = (
            attempt["reportUnlockedAt"] is not None
            and current_lab_score >= unlock_threshold
        )
...
async def student_submit_certification_flag(
    ...
    async with pool.acquire() as conn:
        ...
        if attempt["status"] not in ("LAB_IN_PROGRESS", "REPORT_PENDING"):
            raise HTTPException(
                status_code=400,
                detail=f"Cannot submit flags in status: {attempt['status']}",
            )
        ...
        if data.challenge_id not in challenge_ids:
            raise HTTPException(
                status_code=400, detail="Challenge is not in your assigned pool"
            )
        ...
        if any(
            c["challenge_id"] == data.challenge_id and c.get("solved_at")
        ...
        if is_main_flag_only:
            ...
            if not correct:
                return {
                    ...
                    "points": 0,
                    "lab_points_earned": attempt["labPointsEarned"],
                    "lab_score": float(attempt["labScore"])
                    if attempt["labScore"]
                    else 0,
                    "report_unlocked": attempt["reportUnlockedAt"] is not None,
                }
            ...
            if qi is None:
                raise HTTPException(
                    status_code=400,
                    detail="question_index is required for multi-question challenges",
                )
            if qi < 0 or qi >= len(questions):
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid question index: {qi}. Challenge has {len(questions)} questions.",
                )
        ...
        difficulty = (
            challenge["difficulty"].upper() if challenge["difficulty"] else "MEDIUM"
        )
        cert_points = CERTIFICATION_DIFFICULTY_POINTS.get(difficulty, 20)

        # Find or create the progress entry for this challenge
        existing_entry = next(
            (c for c in solved_challenges if c["challenge_id"] == data.challenge_id),
            None,
        )
        ...
        if total_pts == 0:
            ...
            total_pts = (
                sum(
                    CERTIFICATION_DIFFICULTY_POINTS.get(
                        ch["difficulty"].upper() if ch["difficulty"] else "MEDIUM", 20
                    )
                    for ch in pool_challenges
                )
                or 1
            )
        ...
        all_solved = all(
            c.get("solved_at") is not None for c in solved_challenges
        ) and len(solved_challenges) == len(challenge_ids)
        ...
        return {
            ...
            "challenges_solved": sum(
                1 for c in solved_challenges if c.get("solved_at")
            ),
            "challenges_total": 7,
...
async def student_end_certification_lab(
    attempt_id: str, current_user: dict = Depends(get_current_user)
):
    """
    ...
    async with pool.acquire() as conn:
        ...
        if attempt["status"] not in ("LAB_IN_PROGRESS",):
            raise HTTPException(
                status_code=400,
                detail=f"Lab cannot be ended in status: {attempt['status']}",
            )
...
async def student_get_certification_exam_status(
    ...
    async with pool.acquire() as conn:
        ...
        return {
            ...
            "mcq_wrong": (attempt["mcqTotal"] or 0) - (attempt["mcqCorrect"] or 0)
            if attempt["mcqTotal"]
            else None,
            ...
            "report_score": float(attempt["reportTotalScore"])
            if attempt["reportTotalScore"]
            else None,
            "final_score": float(attempt["finalScore"])
            if attempt["finalScore"]
            else None,
            "certification_level": attempt["certificationLevel"],
            "global_timer_end": attempt["globalExpiresAt"].isoformat()
            if attempt["globalExpiresAt"]
            else None,
            "lab_timer_end": attempt["labExpiresAt"].isoformat()
            if attempt["labExpiresAt"]
            else None,
            "report_timer_end": None,  # Report timer removed
            "report_uploaded_at": attempt["reportUploadedAt"].isoformat()
            if attempt["reportUploadedAt"]
            else None,
            "graded_at": attempt["reportGradedAt"].isoformat()
            if attempt["reportGradedAt"]
            else None,
            "grader_comments": attempt["reportFeedback"]
            if attempt["reportFeedback"]
            else None,
            "created_at": attempt["redeemedAt"].isoformat()
...
async def student_get_report_status(
    ...
    async with pool.acquire() as conn:
        ...
        return {
            ...
            "attempt_id": str(attempt["id"]),
            "lab_score": current_lab_score,
            "can_upload_report": report_unlocked
            and attempt["reportUploadedAt"] is None,
            "report_uploaded_at": attempt["reportUploadedAt"].isoformat()
            if attempt["reportUploadedAt"]
            else None,
...
async def admin_reset_certification_attempt(
    attempt_id: str, current_user: dict = Depends(get_current_user)
):
    """
    ...
    async with pool.acquire() as conn:
        ...
        return {
            "success": True,
            "message": "Attempt reset to post-MCQ state. Student can now restart the lab.",
        }
...
async def student_upload_certification_report(
    ...
    async with pool.acquire() as conn:
        ...
        if attempt["reportUnlockedAt"] is None:
            raise HTTPException(
                status_code=400,
                detail="Report upload is not unlocked. You need at least 80% lab score.",
            )
        ...
        if attempt["status"] not in (
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Cannot upload report in status: {attempt['status']}",
            )
        ...
        if report_expires:
            ...
            if now >= report_expires:
                ...
                raise HTTPException(
                    status_code=400, detail="Your report upload deadline has expired"
                )
        ...
        return [
            {
                ...
                "report_uploaded_at": r["reportUploadedAt"].isoformat()
                if r["reportUploadedAt"]
                else None,
            }
            for r in reports
        ]
...
async def admin_get_report_details(
    attempt_id: str, admin: dict = Depends(require_admin)
):
    ...
    async with pool.acquire() as conn:
        ...
        return {
            ...
            "report": {
                ...
                "uploaded_at": attempt["reportUploadedAt"].isoformat()
                if attempt["reportUploadedAt"]
                else None,
                ...
                "grading": {
                    ...
                    "total": float(attempt["reportTotalScore"])
                    if attempt["reportTotalScore"]
                    else None,
            ...
            # Timing
            "timing": {
                "redeemed_at": attempt["redeemedAt"].isoformat()
                if attempt["redeemedAt"]
                else None,
                "global_expires_at": attempt["globalExpiresAt"].isoformat()
                if attempt["globalExpiresAt"]
                else None,
                "lab_started_at": attempt["labStartedAt"].isoformat()
                if attempt["labStartedAt"]
                else None,
                "lab_completed_at": attempt["labCompletedAt"].isoformat()
                if attempt["labCompletedAt"]
                else None,
                "report_unlocked_at": attempt["reportUnlockedAt"].isoformat()
                if attempt["reportUnlockedAt"]
                else None,
                "report_expires_at": attempt["reportExpiresAt"].isoformat()
                if attempt["reportExpiresAt"]
                else None,
            },
        }
...
async def admin_grade_certification_report(
    attempt_id: str, data: ReportGradeRequest, admin: dict = Depends(require_admin)
):
    """
    ...
    async with pool.acquire() as conn:
        ...
        if not attempt["reportFileUrl"]:
            raise HTTPException(
                status_code=400, detail="No report uploaded for this attempt"
            )
...
async def admin_send_notification(
    data: NotificationCreate, admin: dict = Depends(require_admin)
):
...
async def start_docker_instance(
    challenge_id: str, current_user: dict = Depends(get_current_user)
):
    ...
    async with pool.acquire() as conn:
        ...
        if not challenge["docker_image"] and not challenge["challengePackId"]:
            raise HTTPException(
                status_code=400, detail="This challenge does not have a container"
            )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if spawn_resp.status_code != 201:
                    ...
                    raise HTTPException(
                        status_code=503, detail=f"Nexus error: {error_detail}"
                    )
                ...
                try:
                    ...
                    async with pool.acquire() as conn:
                        ...
                        """)
                        logger.info(
                            f"[NEXUS-INSERT] Saving session: user_id='{user_id}', challenge_id='{challenge_id}', session_id='{session_data['session_id']}'"
                        )
                        await conn.execute(
                            """
                    ...
                    logger.error(
                        f"[NEXUS-INSERT] Failed to record usage: {e}"
                    )  # Don't fail the request

                # Support both pod_ip (v3.0 VPN mode) and target_ip (legacy)
                instance_ip = session_data.get("pod_ip") or session_data.get(
                    "target_ip"
                )
                return {
            ...
            raise HTTPException(
                status_code=504,
                detail="Nexus timeout - container may still be starting",
            )
...
async def extend_docker_instance(
    session_id: str, current_user: dict = Depends(get_current_user)
):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if status_resp.status_code == 200:
                ...
                if expires_at_str:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Extension check for {session_id}: {remaining_minutes:.1f} minutes remaining"
                        )
                        ...
                        logger.warning(
                            f"Could not parse expires_at '{expires_at_str}': {e}"
                        )
            ...
            if resp.status_code == 200:
                ...
                return {
                    ...
                    "expires_at": result.get("new_expires_at")
                ...
                raise HTTPException(
                    status_code=403, detail="Extension not available yet"
                )
            elif resp.status_code == 404:
                raise HTTPException(status_code=404, detail="Session not found")
            else:
                ...
                raise HTTPException(
                    status_code=resp.status_code,
                    detail=f"Nexus error: {resp.status_code}",
                )
    except HTTPException:
        raise
    except httpx.RequestError as e:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                raise HTTPException(
                    status_code=resp.status_code, detail="Failed to get status"
                )
    except httpx.RequestError as e:
        raise HTTPException(status_code=503, detail="Nexus Engine unavailable")
...
async def get_challenge_session(
    ...
    try:
        ...
        async with pool.acquire() as conn:
            logger.info(
                f"[NEXUS-SELECT] Looking for session: user_id='{user_id}', challenge_id='{challenge_id}'"
            )
            row = await conn.fetchrow(
                """
            ...
            if row:
                session_id = row["session_id"]
                logger.info(
                    f"[NEXUS-SELECT] Found running session {session_id} in DB for user {user_id}"
                )
                ...
                try:
                    async with httpx.AsyncClient() as client:
                        ...
                        if resp.status_code == 200:
                            ...
                            logger.info(
                                f"Session {session_id} not in Nexus (404) - marking expired"
                            )
                            ...
                            logger.warning(
                                f"Nexus returned {resp.status_code} for {session_id} - trusting DB"
                            )
                except Exception as e:
                    # Any error (timeout, connection, etc) - trust database
                    logger.warning(
                        f"Nexus check failed for {session_id}: {e} - trusting DB"
                    )
    ...
    logger.info(
        f"[NEXUS-SELECT] Returning status=none for user {user_id}, challenge {challenge_id}"
    )
    return {"status": "none"}
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            sess_resp = await client.get(
                f"{NEXUS_ENGINE_URL}/api/v1/sessions", timeout=10.0
            )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            for row in rows:
                ...
                is_orphaned = (
                    row["status"] == "running"
                    and session_id not in nexus_sessions
                    and len(nexus_sessions) > 0
                )
                ...
                sessions.append(
                    {
                        ...
                        "status": row["status"],
                        "target_ip": nexus_data.get("pod_ip")
                        or nexus_data.get("target_ip"),  # Support v3.0 VPN mode
...
async def admin_terminate_session(
    session_id: str, current_user: dict = Depends(require_admin)
):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                return {
                    "success": True,
                    "message": f"Session marked as terminated (Nexus status: {resp.status_code})",
                }
        ...
        return {
            "success": True,
            "message": "Session marked as terminated (Nexus unavailable)",
        }
...
async def admin_cleanup_orphans(current_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        k8s_session_ids = [
            s.replace("svc-", "") for s in active_services if s.startswith("svc-sess-")
        ]
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT session_id FROM nexus_usage WHERE status = 'running'"
            )
            ...
            await asyncio.create_subprocess_shell(
                "kubectl delete pods -n nexus-challenges --field-selector=status.phase!=Running"
            )
                    ...
                    try:
                        async with httpx.AsyncClient() as client:
                            ...
                            if resp.status_code == 404:
                                ...
                                logger.info(
                                    f"🛡️ Janitor: Session {session_id} not found in Nexus, marking as expired"
                                )
                                # Calculate cost when marking as expired
                                await conn.execute(
                                    """
                        ...
                        logger.debug(
                            f"🛡️ Janitor: Could not verify session {session_id}: {e}"
                        )
...
async def admin_nexus_history(
    page: int = 1, limit: int = 50, current_user: dict = Depends(require_admin)
):
...
async def admin_nexus_pricing(
    ...
    return {
        "pricing": {
            ...
            "breakdown": {
                "vcpu_0.25": round(vcpu_cost, 4),
                "memory_0.5gb": round(memory_cost, 5),
        ...
        "monthly_projection": {
            "assuming_daily_usage": True,
            "monthly_instance_hours": monthly_hours,
...
async def admin_nexus_bigquery_billing(
    days: int = 30, current_user: dict = Depends(require_admin)
):
    """
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code == 200:
                ...
                logger.warning(
                    f"Nexus billing returned {resp.status_code}: {resp.text}"
                )
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            try:
                ...
                logger.debug(
                    f"User profile columns already exist or migration skipped: {e}"
                )
            ...
            try:
                ...
                logger.debug(
                    f"OAuth avatar columns already exist or migration skipped: {e}"
                )
            ...
            try:
                ...
                logger.debug(
                    f"Multi-container pack columns already exist or migration skipped: {e}"
                )
            ...
            try:
                ...
                logger.debug(
                    f"Module quiz tables already exist or migration skipped: {e}"
                )
...
async def get_active_features(
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT key, name, description, status FROM feature_flags"
        )
...
async def create_feature_flag(
    flag: FeatureFlagCreate, admin: dict = Depends(require_superadmin)
):
    """Create a new feature flag (superadmin only)"""
    if flag.status not in ("disabled", "beta", "enabled"):
        raise HTTPException(
            status_code=400, detail="Status must be: disabled, beta, or enabled"
        )
    ...
    async with pool.acquire() as conn:
        existing = await conn.fetchrow(
            "SELECT id FROM feature_flags WHERE key = $1", flag.key
        )
        if existing:
            raise HTTPException(
        ...
        logger.info(
            f"Feature flag created: {flag.key} ({flag.status}) by {admin.get('username')}"
        )
        return dict(row)
...
async def update_feature_flag(
    flag_key: str, update: FeatureFlagUpdate, admin: dict = Depends(require_superadmin)
):
    ...
    async with pool.acquire() as conn:
        ...
        logger.info(
            f"Feature flag updated: {flag_key} -> {new_status} by {admin.get('username')}"
        )
        return dict(row)
...
async def delete_feature_flag(flag_key: str, admin: dict = Depends(require_superadmin)):
    ...
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM feature_flags WHERE key = $1", flag_key
        )
        if result == "DELETE 0":
            raise HTTPException(status_code=404, detail="Feature flag not found")
...
async def force_logout_session(
    session_id: str, admin: dict = Depends(require_superadmin)
):
    """Force logout a specific session (superadmin only)"""
    pool = await Database.get_pool()
    async with pool.acquire() as conn:
        result = await conn.execute(
            "UPDATE ctf_active_sessions SET is_active = false WHERE id::text = $1",
            session_id,
        )

        if result == "UPDATE 0":
            raise HTTPException(
                status_code=404, detail="Session not found or already inactive"
            )
